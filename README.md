...
給管理 100+ 個房地產相關 LINE 群組使用的新聞蒐集 + 自動發文系統。同一個伺服器
也附帶「買方客需媒合看板」，詳見本文件最後一節與
[`docs/buyer-demand-board.md`](docs/buyer-demand-board.md)。
...
同一個伺服器上另外提供 `public/buyer-board.html`（`http://localhost:3000/buyer-board.html`），
讓聯盟房仲刊登買方客需、供同業與買家自由瀏覽配對，右側提供廣告版位。完整的
MVP 可行性分析、資料模型與 API 設計見 [`docs/buyer-demand-board.md`](docs/buyer-demand-board.md)。
...
- **廣告版位**：右側顯示上架中的廣告（依排序、效期自動顯示/下架），管理者
  輸入 `ADMIN_TOKEN` 後可在同一頁新增/編輯/下架廣告，並登打「已收費/未收費」
  狀態；每則廣告會記錄曝光與點擊次數。
