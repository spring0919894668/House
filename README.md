...
| 實價合理性分析 | 依「同地區＋同類型」的系統內案件與手動輸入的比較案例試算合理總價區間，並可透過 `.env` 的 `PRICE_AI_ENDPOINT` 串接正式的實價登入 AI 服務（介面在 `src/matching/priceEngine`，格式不變即可平行替換） |
...
src/matching/
  ...
  priceEngine/       # 實價分析轉接層（可切換外部 API 或本地試算）
